window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function foldAttachmentDiv(div) {
      ...
      // Never fold inside the composer: the draft is editable user input.
      // Folding it hides the text behind read-only chips that cannot be
      // selected or removed, which made leftover paste blocks look like
      // un-deletable messages (2026-08-17 incident).
      if (div.closest('[contenteditable="true"], [contenteditable=""], [contenteditable="plaintext-only"], textarea') !== null) return;
    ...
    function foldScan() {
      ...
      while ((node = walker.nextNode()) !== null) {
        ...
        // Same guard as foldAttachmentDiv: never fold editable composer text
        // (leftover draft blocks must stay visible and removable).
        if (container.closest('[contenteditable="true"], [contenteditable=""], [contenteditable="plaintext-only"], textarea') !== null) continue;
    ...
    function apply(ctx) {
      ...
      // One-time recovery: drafts persisted with a leftover DSH_PASTE_INPUT_V1
      // block (a paste that was never sent, or a failed send that kept its
      // serialized text in the composer) used to get folded into read-only
      // chips, making them look like un-deletable messages. DSH restores the
      // draft from this storage after client plugins apply, so clearing it
      // here gives every conversation a clean composer on next load.
      try {
        for (let i = 0; i < localStorage.length; i += 1) {
          const key = localStorage.key(i);
          if (key === null || !key.includes('dsh.conversation.chat.')) continue;
          try {
            const value = JSON.parse(localStorage.getItem(key) ?? '');
            if (value !== null && typeof value === 'object' && typeof value.draft === 'string' && value.draft.includes('DSH_PASTE_INPUT_V1')) {
              value.draft = '';
              localStorage.setItem(key, JSON.stringify(value));
              console.warn('[dsh-paste-input] cleared leftover attachment draft in', key);
            }
          } catch { /* not a conversation state value */ }
        }
      ...
      const onPaste = event => {
        ...
        // Capture-phase interception: block other paste listeners (e.g.
        // vision-toolkit takeover) from swallowing clipboard files first.
      ...
      ctx.effect(() => () => document.removeEventListener('paste', onPaste, true), 'dsh-paste-input: paste listener');
